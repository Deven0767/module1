...
{   
    ...
    UART_TransferGetReceiveCountEDMA(DEMO_UART, &g_uartEdmaHandle, &count);
    UART_WriteBlocking(DEMO_UART, buffer, 2);
    receivecomplete(&g_uartRxEdmaHandle);
    UART_ClearStatusFlags(DEMO_UART, kUART_IdleLineFlag);
}
...
int main(void)
{
    ...
    UART_EnableInterrupts(DEMO_UART, kUART_IdleLineInterruptEnable);
